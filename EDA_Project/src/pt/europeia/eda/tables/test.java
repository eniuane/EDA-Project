...
public class test {
	...
	public static double executionTimeFor(final int limit, final String fileToSort) {
	...
	public static void performExperimentsFor(final int limit, final boolean isWarmup, final String fileToSort) {
		...
		long repetitions = 0;
		do {
			executionTimes.add(executionTimeFor(limit, fileToSort));
			repetitions++;
		...
		if (!isWarmup) {
			out.println("Put one value in a table with " + limit + " values \t median= " + median + "\t Average= "
